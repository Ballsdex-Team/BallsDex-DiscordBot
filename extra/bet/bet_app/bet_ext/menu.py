...
import discord
from discord.ui import ActionRow, Button, Select, TextDisplay, View, button
...
from .display import fill_match_embed_fields
from .match_user import MatchingUser

if TYPE_CHECKING:
    from ballsdex.core.bot import BallsDexBot

    from .cog import Match as MatchCog
...
class MatchView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction["BallsDexBot"], /) -> bool:
        try:
            ...
            await interaction.response.send_message("You are not allowed to interact with this match.", ephemeral=True)
            return False
        else:
            return True
    ...
    async def lock(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        if player.locked:
            await interaction.response.send_message("You have already locked your bet!", ephemeral=True)
            return
        await interaction.response.defer(thinking=True, ephemeral=True)
        await self.match.lock(player)
        if self.match.player1.locked and self.match.player2.locked:
            await interaction.followup.send(
                "Your bet has been locked. The match will execute automatically!", ephemeral=True
            )
        else:
            await interaction.followup.send(
                "Your bet has been locked. Waiting for the other player to lock their bet...", ephemeral=True
            )
    ...
    async def clear(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        view = ConfirmChoiceView(
            interaction, accept_message="Clearing your bet...", cancel_message="This request has been cancelled."
        )
        await interaction.followup.send("Are you sure you want to clear your bet?", view=view, ephemeral=True)
        await view.wait()
        if not view.value:
            return
    ...
    async def cancel(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        view = ConfirmChoiceView(
            interaction, accept_message="Cancelling the match...", cancel_message="This request has been cancelled."
        )
        await interaction.followup.send("Are you sure you want to cancel this match?", view=view, ephemeral=True)
        await view.wait()
        if not view.value:
            return
...
class MatchMenu:
    ...
    def _generate_embed(self):
        ...
        self.embed.description = (
            ...
            "**The winner gets ALL items from both bets!**\n\n"
        ...
        self.embed.set_footer(text="This message is updated every 15 seconds, but you can keep on editing your bet.")
    ...
    async def start(self):
        ...
        self.message = await self.channel.send(
            content=f"Hey {self.player2.user.mention}, {self.player1.user.name} is challenging you to a 50/50 match!",
            embed=self.embed,
...
class MatchViewMenu(LayoutView):
    """View for displaying a player's current match bet."""

    def __init__(self, cog: "MatchCog", channel: discord.TextChannel):
        super().__init__()
    ...
    async def selected(self, interaction: discord.Interaction["BallsDexBot"], select: Select):
        ...
        try:
            user = await interaction.client.fetch_user(value)
        ...
        match, player = self.cog.get_match(interaction, user=user)
        if not match or not player:
            await interaction.response.send_message("Match not found for that user.", ephemeral=True)
            return
        embed = discord.Embed(
            title="Your Current Match Bet",
            description=f"You have {len(player.bet)} {settings.plural_collectible_name} in your bet.",
            color=discord.Color.gold(),
        )

        for ball in player.bet:
            embed.add_field(
                name=ball.countryball.country,
                value=ball.description(short=True, include_emoji=True, bot=interaction.client),
                inline=True,
            )
...
class BulkAddView(View):
    ...
    async def add_selected(self, interaction: discord.Interaction["BallsDexBot"], button: Button):
        ...
        if player.locked:
            await interaction.followup.send("Your bet is locked and cannot be edited.", ephemeral=True)
            return
        ...
        await interaction.followup.send(
            f"Added {added_count} {settings.plural_collectible_name} to your bet.", ephemeral=True
        )
        self.stop()
