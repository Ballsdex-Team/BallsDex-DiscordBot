...
from .menu import BulkAddView, MatchMenu, MatchViewMenu
...
class Match(commands.GroupCog):
    ...
    def get_match(
        ...
        *,
        ...
        for match in self.matches[guild.id][channel.id]:
            if match.current_view.is_finished() or match.player1.cancelled or match.player2.cancelled:
    ...
    async def begin(self, interaction: discord.Interaction["BallsDexBot"], user: discord.User):
        ...
        if user.id == interaction.user.id:
            await interaction.response.send_message("You cannot match with yourself.", ephemeral=True)
            return
        ...
        if match1 or player1_obj:
            await interaction.response.send_message("You already have an ongoing match.", ephemeral=True)
            return
        ...
        if player2.discord_id in self.bot.blacklist:
            await interaction.response.send_message("You cannot match with a blacklisted user.", ephemeral=True)
            return

        menu = MatchMenu(self, interaction, MatchingUser(interaction.user, player1), MatchingUser(user, player2))
    ...
    async def add(
        ...
        if not countryball.is_tradeable:
            await interaction.response.send_message(f"You cannot bet this {settings.collectible_name}.", ephemeral=True)
            return
        ...
        if countryball.favorite:
            ...
            await interaction.followup.send(
                f"This {settings.collectible_name} is a favorite, are you sure you want to bet it?",
                view=view,
                ephemeral=True,
            )
            await view.wait()
        ...
        if countryball in player.bet:
            await interaction.followup.send(
                f"You already have this {settings.collectible_name} in your bet.", ephemeral=True
            )
            return
        ...
        await interaction.followup.send(f"{countryball.countryball.country} added to bet.", ephemeral=True)
    ...
    async def remove(
        ...
        match, player = self.get_match(interaction)
        if not match or not player:
            await interaction.response.send_message("You do not have an ongoing match.", ephemeral=True)
            return
        if player.locked:
            await interaction.response.send_message(
                "You have locked your bet, it cannot be edited! "
        ...
        await interaction.response.send_message(f"{countryball.countryball.country} removed from bet.", ephemeral=True)
    ...
    async def view(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        options = [
            discord.SelectOption(
                label=player1.user.display_name,
                description=f"ID: {player1.user.id} | {len(player1.bet)} {settings.plural_collectible_name}",
                value=str(player1.user.id),
            ),
            discord.SelectOption(
                label=player2.user.display_name,
                description=f"ID: {player2.user.id} | {len(player2.bet)} {settings.plural_collectible_name}",
                value=str(player2.user.id),
            ),
        ]

        view = MatchViewMenu(self, cast(discord.TextChannel, interaction.channel))
        view.restrict_author(interaction.user.id)
        menu = Menu(self.bot, view, ChunkedListSource(options), SelectFormatter(view.selected))
        await menu.init(position=2)
        view.header.content = "Select a user to view their proposal."
    ...
    async def info(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        embed = discord.Embed(
            title="Match Information", description="50/50 winner takes all match", color=discord.Color.gold()
        )
        embed.add_field(
