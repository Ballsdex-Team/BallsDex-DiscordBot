...
class MatchViewFormat(menus.ListPageSource):
    ...
    async def format_page(self, menu: Pages, match: TradeModel) -> discord.Embed:
        ...
        embed.set_footer(text=f"Match {menu.current_page + 1}/{menu.source.get_max_pages()} | Match date: ")
...
def _build_list_of_strings(player: MatchingUser, bot: "BallsDexBot", short: bool = False) -> list[str]:
...
def fill_match_embed_fields(
    ...
    embed.add_field(name=_get_player_name(player1, is_admin), value=player1_bet[0], inline=True)
    embed.add_field(name=_get_player_name(player2, is_admin), value=player2_bet[0], inline=True)
    ...
    if len(embed) > 6000:
        if not compact:
            return fill_match_embed_fields(embed, bot, player1, player2, compact=True, is_admin=is_admin)
        else:
            embed.clear_fields()
            embed.add_field(
                name=_get_player_name(player1, is_admin),
                value=(f"Bet too long, only showing last page:\n{player1_bet[-1]}\nTotal: {len(player1.bet)}"),
                inline=True,
            )
            embed.add_field(
                name=_get_player_name(player2, is_admin),
                value=(f"Bet too long, only showing last page:\n{player2_bet[-1]}\nTotal: {len(player2.bet)}"),
                inline=True,
            )
