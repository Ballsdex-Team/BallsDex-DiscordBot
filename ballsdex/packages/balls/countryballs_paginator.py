...
class CountryballsSource(menus.ListPageSource):
    ...
    async def prepare(self):
        first_entries = self.entries[: self.per_page * 5] if len(self.entries) > self.per_page * 5 else self.entries
        balls = BallInstance.objects.filter(id__in=first_entries)
    ...
    async def fetch_page(self, ball_ids: list[int]) -> AsyncIterator[BallInstance]:
        if ball_ids[0] not in self.cache:
            async for ball in BallInstance.objects.filter(id__in=ball_ids):
                self.cache[ball.pk] = ball
        for id in ball_ids:
            yield self.cache[id]
