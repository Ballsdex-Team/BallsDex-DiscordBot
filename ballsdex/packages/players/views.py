        ...
        component=Select(
            options=[
                ...
                discord.SelectOption(
                    ...
                    value=ExportCategory.TRADES,
                ),
                discord.SelectOption(
                    label="All",
                    description="Export everything.",
                    value=ExportCategory.ALL,
                ),
            ]
        ),
    )
    footer = TextDisplay(
        f"-# Check the [privacy policy]({settings.privacy_policy}) for more informations about "
        "your data."
    )
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            await interaction.user.send(
                "Here is your player data:", file=discord.File(zip_file, "player_data.zip")
            )
            await interaction.followup.send(
    ...
    def configure(self, interaction: Interaction, player: Player):
        ...
        def button_callback(buttons: dict[int, Button], setting: str, value: int):
            async def callback(interaction: Interaction):
                ...
                await player.save(update_fields=(setting,))
        ...
        settings_buttons = [
            ...
            Row(
                ...
                "friend_policy",
                {
                    FriendPolicy.ALLOW: Button(label="Open"),
                    FriendPolicy.DENY: Button(label="Closed"),
                },
            ),
            Row(
                self.trade_actions,
            ...
            Row(
                ...
                "mention_policy",
                {
                    MentionPolicy.ALLOW: Button(label="Mentions"),
                    MentionPolicy.DENY: Button(label="No mentions"),
                },
            ),
        ]
