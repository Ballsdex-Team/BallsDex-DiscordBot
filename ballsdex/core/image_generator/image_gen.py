...
title_font = ImageFont.truetype(str(SOURCES_PATH / "RussoOne-Regular.ttf"), 170)
capacity_name_font = ImageFont.truetype(str(SOURCES_PATH / "BebasNeue-Regular.ttf"), 110)
capacity_description_font = ImageFont.truetype(str(SOURCES_PATH / "HelveticaNeueMedium.otf"), 70)
stats_font = ImageFont.truetype(str(SOURCES_PATH / "BebasNeue-Regular.ttf"), 130)
credits_font = ImageFont.truetype(str(SOURCES_PATH / "HelveticaNeueRoman.otf"), 40)
...
def draw_card(ball_instance: "BallInstance", media_path: str = "./admin_panel/media/"):
    ...
    cap_name = textwrap.wrap(f"Ability: {ball.capacity_name}", width=32)
    ...
    for i, line in enumerate(textwrap.wrap(ball.capacity_description, width=36)):
        draw.text(
